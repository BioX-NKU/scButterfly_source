...
opts, args = getopt.gnu_getopt(sys.argv[1:], 't:d:f:n:ds', ['model_type=', 'data=', 'file_path=', 'number=', 'down_sample_rate='])
model_type = opts[0][1]
data = opts[1][1]
file_path = opts[2][1]
number = opts[3][1]
down_sample_rate = float(opts[4][1])
...
import random
random.seed(19193)
RNA_feature_id = [i for i in range(RNA_data.shape[1])]
ATAC_feature_id = [i for i in range(ATAC_data.shape[1])]
random.shuffle(RNA_feature_id)
random.shuffle(ATAC_feature_id)
RNA_feature_id = RNA_feature_id[0:int(down_sample_rate * len(RNA_feature_id))]
ATAC_feature_id = ATAC_feature_id[0:int(down_sample_rate * len(ATAC_feature_id))]
RNA_feature_id.sort()
ATAC_feature_id.sort()
RNA_data = RNA_data[:, RNA_feature_id]
ATAC_data = ATAC_data[:, ATAC_feature_id]
RNA_data = ad.AnnData(X=csr_matrix(RNA_data.X.todense()), obs = RNA_data.obs, var = RNA_data.var)
ATAC_data = ad.AnnData(X=csr_matrix(ATAC_data.X.todense()), obs = ATAC_data.obs, var = ATAC_data.var)
