...
opts, args = getopt.gnu_getopt(sys.argv[1:], 't:d:f:n:r:a:k:d', ['model_type=', 'data=', 'file_path=', 'number=', 'rna_param=', 'atac_param=', 'kl_param=', 'dis_param='])
model_type = opts[0][1]
data = opts[1][1]
file_path = opts[2][1]
number = opts[3][1]
rna_param = float(opts[4][1])
atac_param = float(opts[5][1])
kl_param = float(opts[6][1])
dis_param = float(opts[7][1])
...
R_kl_div = 1 / RNA_input_dim * kl_param
A_kl_div = 1 / ATAC_input_dim * kl_param
...
model.train(
    ...
    loss_weight = [rna_param, atac_param, dis_param, R_kl_div, A_kl_div, kl_div],
